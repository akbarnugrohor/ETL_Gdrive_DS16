...
def __download_file(service, file_id, file_name):
    ...
    with open(file_name, "wb") as f:
        f.write(fh.head())
    print(f"Download file {file_name} from Google Drive.")
...
def _check_file_if_exist(ti):
    ...
    print(f"The '{dataset_folder_name}' files exist in 'Shared with me'.")
...
def _process_csv(ti):
    ...
    # pull xcom
    dataset_files = ti.xcom_pull(key="dataset_files", task_id="check_file_if_exist")
...
def _upsert_data(ti):
    ...
    # CSV file path
    # pull xcom
    list_processed_name = ti.xcom_pull(key="list_processed_name", task_id="process_csv")
    
    for file in list_processed_name:
        # Connect to PostgreSQL
        conn = psycopg2.connect(**db_params)
        cur = conn.cursor()
        # Load data into PostgreSQL usin SQLAlchemy
